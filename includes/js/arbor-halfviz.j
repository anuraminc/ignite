...
//(function(){
  ...
  //    io:IO("#editor .io"),
      ...
      getDoc:function(e){
        $.getJSON(//'library/'+e.id+'.json', 
        '../includes/data/arbor-halfviz/toc.json',
          function(doc){
      ...
      updateGraph:function(e){
        ...
        sys.parameters({ friction : (count < 5 ? 1.0 : 0.5) });
...
//    //mcp.newDoc();
//    mcp.loadData("; ssome example nodes\nvenkat {color:red, label:HELLO, link:'http://www.google.com'}\nworld {color:orange}\n\n; some edges\nhello -> world {color:yellow}\nfoo -> bar {weight:5}\nbar -> baz {weight:2}");
//  })
